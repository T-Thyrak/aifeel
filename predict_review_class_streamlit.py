...
vectorizer_for_multi = model_file.load(open('export/model/TFIDFModelClassifier/vectorizer.dill', 'rb'))
...
def visualize_probabilities(positive_prob, negative_prob):
    ...
    ax.legend(labels, loc="upper right", bbox_to_anchor=(0.5, 0, 0.5, 1)) # location of legend 1=right, 2=left, 3=upper, 4=lower
...
def main():
    ...
    if st.button('Predict'):
        ...
        if model_selection == "NNClassifier":
            ...
            text_feature = extract_features(text_review, positive_words, negative_words, vectorizer=vectorizer)
